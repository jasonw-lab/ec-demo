<template>
  <div>
    ...
    <!-- 空キーワード時の表示 -->
    <div
      v-if="!q || q.trim().length === 0"
      style="text-align: center; padding: 48px; color: #6b7280"
    >
    ...
    <div v-else>
          ...
          <div style="padding: 16px; display: flex; flex-direction: column; gap: 12px; flex: 1">
            ...
            >
              ...
              <HoverHint text="追加後は、右上のカートアイコンをクリックして会計へお進みください。">
                <button
                  :data-tour="idx === 0 ? 'add-to-cart-primary' : undefined"
                  style="
                    background: #ff6b6b;
                    color: white;
                    border: none;
                  ...
                  "
                  @click="add(p)"
                  @mouseenter="(e) => (e.currentTarget.style.backgroundColor = '#e55555')"
                  @mouseleave="(e) => (e.currentTarget.style.backgroundColor = '#ff6b6b')"
                >
                  カートに追加
                </button>
              </HoverHint>
      ...
      <!-- 検索結果が少ない場合の提案 -->
      <div
        v-if="results.length < 5 && results.length > 0"
        style="
          margin-top: 32px;
          background: #f8fafc;
          border-radius: 12px;
