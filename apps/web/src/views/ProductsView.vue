<template>
  <div>
    ...
    >
      ...
      >
        ...
        <div>
          <select
            v-model="sortKey"
            style="padding:8px 12px;border:1px solid #e5e7eb;border-radius:8px;font-size:14px;outline:none;focus:border-color:#ff6b6b;"
          >
    ...
    >
      ...
      >
        <div
          ...
          :key="p.id"
          style="border:1px solid #f1f5f9;border-radius:12px;overflow:hidden;background:#fff;display:flex;flex-direction:column;transition:transform 0.2s;hover:transform:translateY(-2px);"
        >
          <img
            :src="p.imageUrl || getImageUrl('/product.svg')"
            alt="商品画像"
            style="width: 100%; height: 200px; object-fit: cover; background: #f9fafb"
          />
          <div style="padding: 16px; display: flex; flex-direction: column; gap: 12px; flex: 1">
            ...
            <div
              ...
              "
            >
              <div style="color: #ff6b6b; font-weight: 700; font-size: 18px">
                ¥ {{ Number(p.price).toLocaleString() }}
              </div>
              <HoverHint text="追加後は、右上のカートアイコンをクリックして会計へお進みください。">
                <button
                  style="background:#ff6b6b;color:white;border:none;border-radius:8px;padding:8px 16px;cursor:pointer;font-weight:600;transition:background-color 0.2s;hover:background:#e55555;"
                  @click="add(p)"
                >
                  カートに追加
                </button>
    ...
    <Pagination :page="pageIndex" :total-pages="totalPages" @change="goToPageIndex" />
  </div>
</template>
    ...
    : [
        {
          ...
          description: 'サンプル',
          price: 1000,
          imageUrl: getImageUrl('/product.svg'),
        } as Product,
      ];
  let i = 0;
...
function categoryChipStyle(c: Category): string {
  ...
  return (
    ...
    'border-radius:999px;padding:8px 12px;text-align:center;text-decoration:none;display:block;transition:all 0.2s;hover:border-color:#ff6b6b;'
  );
}
</script>
