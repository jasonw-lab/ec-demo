    ...
    <div v-else>
      ...
      >
        <div
          ...
          :key="c.id"
          style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);transition:transform 0.2s;hover:transform:translateY(-2px);"
        >
          <router-link
            :to="`/category/${c.id}`"
          ...
          >
            <img
              :src="c.imageUrl || getImageUrl('/product.svg')"
              alt="カテゴリ画像"
              style="width: 100%; height: 120px; object-fit: cover; background: #f9fafb"
            />
      ...
      <div style="margin-top: 32px; background: #f8fafc; border-radius: 12px; padding: 24px">
        <h3 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600; color: #374151">
          カテゴリについて
        </h3>
