package com.demo.ec.alert.service;

import com.demo.ec.alert.eventcontracts.AlertRaisedEvent;
import com.demo.ec.alert.eventcontracts.ObjectMapperProvider;
import com.demo.ec.alert.repository.AlertJdbcRepository;
...
public class AlertProcessService {
    ...
    public void handle(AlertRaisedEvent event) {
        if (event.getAlertId() == null || event.getOrderId() == null || event.getRule() == null) {
            log.warn(
                "skip alert due to missing required fields: alertId={}, orderId={}, rule={}",
                event.getAlertId(),
                event.getOrderId(),
                event.getRule());
            return;
