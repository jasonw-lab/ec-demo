#!/usr/bin/env bash
# =============================================================================
# Initialize OpenSearch products index and import sample data
# =============================================================================
set -euo pipefail
...
echo "============================================="
echo "OpenSearch Products Index Setup"
echo "============================================="
echo "OS_BASE_URL:     ${OS_BASE_URL}"
...
if curl -fsS "${OS_BASE_URL}/${INDEX_VERSIONED}" >/dev/null 2>&1; then
  echo "Index already exists: ${INDEX_VERSIONED} (skip)"
else
  curl -fsS -X PUT "${OS_BASE_URL}/${INDEX_VERSIONED}" \
...
curl -fsS -X POST "${OS_BASE_URL}/_aliases" \
  -H 'Content-Type: application/json' \
  -d @- <<JSON
{
  "actions": [
...
if [ ! -f "${CSV_FILE}" ]; then
  ...
  if [ -n "${BULK_DATA}" ]; then
    RESULT=$(echo -e "${BULK_DATA}" | curl -fsS -X POST "${OS_BASE_URL}/_bulk" \
...
DOC_COUNT=$(curl -fsS "${OS_BASE_URL}/${INDEX_VERSIONED}/_count" | grep -o '"count":[0-9]*' | grep -o '[0-9]*')
